...
module "adls_container" {
  source = "../../../Modules/adls/container"
  count = length(local.storage_containers_name)
  storage_account_name = local.storage_account_name
  container_access_type = local.container_access_type
  storage_container_name = element(local.storage_containers_name,count.index)
...
module "private_endpoint_tfstate_blob" {
  source                               = "../../../Modules/networking/privateEndpoint"
  resource_group_name                  = local.resource_group_name
  location                             = local.location
  private_endpoint_name                = local.tfstate_blob_private_endpoint_name
  subnet_endpoint_id                   = data.azurerm_subnet.pep_subnet.id
  private_service_connection_name      = local.private_service_connection_name
