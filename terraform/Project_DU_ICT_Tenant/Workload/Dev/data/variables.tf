...
# Specifies the kind of Cosmos DB account to create
variable "cosmos_nosql_account_kind" {
  type        = string
  description = "The type of Cosmos DB account to create. Examples include 'GlobalDocumentDB' for NoSQL, 'MongoDB' for MongoDB API, etc."
}
...
variable "cosmos_mongodb_tags" {
  description = "A map of key-value pairs to assign as tags to the Cosmos DB MongoDB instance for resource management and categorization."
  type        = map(string)
}

# Tags for the Cosmos DB NoSQL instance
variable "cosmos_nosql_tags" {
  description = "A map of key-value pairs to assign as tags to the Cosmos DB NoSQL instance for resource management and categorization."
  type        = map(string)
}

# Tags for the Azure Data Lake Storage (ADLS) resource
variable "adls_tags" {
  description = "A map of key-value pairs to assign as tags to the Azure Data Lake Storage (ADLS) resource for better organization and tracking."
  type        = map(string)
}

# Tags for the Azure Key Vault instance
variable "keyvault_tags" {
  description = "A map of key-value pairs to assign as tags to the Azure Key Vault instance for resource identification and grouping."
  type        = map(string)
}
