...
module "subnet_python_app_service" {
  ...
  subnet_address_prefixes                       = var.snet1_address_prefix
...
module "subnet_node_app_service" {
  ...
  subnet_address_prefixes                       = var.subnet2_address_prefix
  subnet_rg_name                                = local.resource_group_name
  virtual_network_name                          = local.virtual_network_name
  subnet_delegations                            = local.subnet_delegation
  service_endpoints                             = local.subnet_endpoints_null
...
# ......................................................
# Creation of Subnet for Vnet Integration MOHAP Python APP service
# ......................................................

module "subnet_mohap_python_app_service" {
  source                                        = "../../../Modules/networking/subnet"
  subnet_name                                   = local.mohap_py_subnet_name
  subnet_address_prefixes                       = var.subnet_mohap_python_address_prefix
  subnet_rg_name                                = local.resource_group_name
  virtual_network_name                          = local.virtual_network_name
  subnet_delegations                            = local.subnet_delegation
  service_endpoints                             = local.subnet_endpoints_null
  ...
  private_endpoint_network_policies             = local.private_endpoint_network_policies_disabled
  rt_id                                         = module.route_table_python_app.route_table_id
  nsg_id                                        = module.mohap_nsg_py_app.nsg_id
  depends_on                                    = [module.vnet, module.route_table_python_app, module.mohap_nsg_py_app]
}
...
module "mohap_nsg_py_app" {
  source       = "../../../Modules/networking/networkSecurityGroup"
  nsg_name     = local.mohap_py_app_nsg_name
  nsg_location = local.location
  nsg_rg_name  = local.resource_group_name
  sec_rule     = local.mohap_py_app_nsg_security_rule
  nsg_tags     = var.nsg_tags
  depends_on   = [module.rg]
}