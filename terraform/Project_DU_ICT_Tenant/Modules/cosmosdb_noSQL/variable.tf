# Location where the Cosmos DB account and noSQL database will be deployed (e.g., UAE North)
variable "location" {
  type        = string
...
variable "cosmosdb_account_kind" {
  type        = string
  description = "The kind of CosmosDB account to be created. Examples include 'GlobalDocumentDB', 'noSQL', etc."
}
...
# Name of the noSQL database to be created within the Cosmos DB account
variable "nosql_database_name" {
  description = "Name of the noSQL database to be created within the Cosmos DB account."
  type        = string
}
