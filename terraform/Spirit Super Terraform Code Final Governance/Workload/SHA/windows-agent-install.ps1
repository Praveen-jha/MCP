...
param (
    [string]$DEVOPSURL,
    [string]$DEVOPSTENANTID,
    [string]$DEVOPSCLIENTID,
    [string]$DEVOPSCLIENTSECRET,
    [string]$DEVOPSPOOL,
...
.\config.cmd --unattended --url "$DEVOPSURL" --auth sp --tenantid "$DEVOPSTENANTID" --clientid "$DEVOPSCLIENTID" --clientsecret "$DEVOPSCLIENTSECRET" --pool "$DEVOPSPOOL" --agent $AGENT_NAME --acceptTeeEula --runAsService

Invoke-WebRequest -Uri https://aka.ms/installazurecliwindows -OutFile .\AzureCLI.msi; Start-Process msiexec.exe -Wait -ArgumentList '/I AzureCLI.msi /quiet'

# Define the version of Terraform you want to install
$terraformVersion = "1.8.1"
$terraformZip = "terraform_1.8.1_windows_amd64.zip"
$terraformUrl = "https://releases.hashicorp.com/terraform/1.8.1/$terraformZip"
