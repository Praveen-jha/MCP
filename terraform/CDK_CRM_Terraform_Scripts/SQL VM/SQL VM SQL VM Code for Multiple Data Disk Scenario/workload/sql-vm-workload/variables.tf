...
variable "sql_disks_simplified_config" {
  description = "A simplified configuration for SQL data, log, and tempdb disks."
  type = object({
    data = object({
...
variable "virtual_machine_config" {
  type = object({
    ...
    windows_vm_availability_zone  = bool   //Enable availability zone deployment.
    windows_vm_vm_zone            = string //Availability zone number.
  })
}
