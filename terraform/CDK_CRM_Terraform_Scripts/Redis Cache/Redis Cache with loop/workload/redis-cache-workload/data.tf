#Data block for Existing Resource Group for Redis Cache
data "azurerm_resource_group" "existing_resource_group_redis_cache" {
  count = var.name_config.redis_cache_resource_group_creation == "existing" ? 1 : 0
  name  = var.existing_resource_group_redis_cache_name
}
...
data "azurerm_subnet" "existing_outbound_subnet" {
  count                = (var.name_config.subnet_creation == "existing" && var.redis_cache_vnet_integration_enable) ? 1 : 0
