# Variables.tf
# This file defines all the input variables required for deploying Azure Service Bus Namespaces 
# along with optional private endpoints using Terraform. It supports flexible deployment scenarios, 
# including creation or reuse of existing resource groups, virtual networks, and subnets.
...
variable "servicebus_mapping" {
  ...
  type = map(object({
    ...
    servicebus_namespace = object({
      ...
      premium_messaging_partitions = number //(Required) Number of messaging partitions for Premium SKU. Must be between 1 and 4.
      local_auth_enabled           = bool   //(Optional) Whether local authentication is enabled.
      minimum_tls_version          = string //(Required) The minimum TLS version to support. One of: 1.0, 1.1, 1.2.
    })
  }))
}
